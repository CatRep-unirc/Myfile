...
public class EventServlet extends EventSourceServlet {
	...
	private synchronized void sendEvent(String event, String data, String endpoint) {
		...
		///////////Esercitazione/////////// 
		try {
			if (!event.equals(EVENT_COAP_LOG)) {
				sendEventToVO(event, data, endpoint);
			}
			else {
				sendEventToVO(event, data, endpoint);
			}
	...
	//////////Esercitazione////////////
	private void sendEventToVO(String event, String data, String endpoint) throws Exception {
		String body = new StringBuffer("{\"event\":\"").append(event + "\n").append("\",\"data\":").append(data  + "\n").append("}")
				.toString();
		// getVOURL get VO address bind to this specific endpoint
		System.out.println("Endpoint :" + endpoint);
		String appURL= "http://localhost:8080";
		//String voURL = BindServlet.getVOURL(endpoint);
		//System.out.println("VO URL :" + voURL);
		try {
			URL fwdURL;
			fwdURL = new URL(appURL);
			// fwdURL = new URL("http://localhost:8080/api/events/");
			HttpURLConnection fwdConnection = (HttpURLConnection) fwdURL.openConnection();
			fwdConnection.setRequestMethod("POST");
			fwdConnection.setRequestProperty("Content-Type", "application/json");
			if (body != null) {
				fwdConnection.setDoOutput(true);
				OutputStreamWriter out = new OutputStreamWriter(fwdConnection.getOutputStream());
				out.write(body);
				out.close();
				System.out.println("body :" + body);
				Reader in = new BufferedReader(new InputStreamReader(fwdConnection.getInputStream(), "UTF-8"));
				StringBuilder sb = new StringBuilder();
				for (int c; (c = in.read()) >= 0;)
					sb.append((char) c);
				String response = sb.toString();
				// System.out.println("Response:" + response);
				LOG.debug("Dispatching event to VO: " + response);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
